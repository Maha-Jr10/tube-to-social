...
// src/trigger/content-os/copywriter-agent.ts
init_esm();
var SYSTEM = `You are an expert copywriter specializing in LinkedIn, X (Twitter), and email newsletters. Write high-converting content grounded in specific source material. Return ONLY valid JSON matching the schema exactly — no markdown fences, no preamble, just the raw JSON object.`;
async function generateLinkedInPosts(payload) {
  ...
  ].map((h) => `[${h.platform}] ${h.text}`).join("\n");
  const coreIdeasSummary = kb.coreIdeas.slice(0, 6).map((c) => `- ${c.idea}: ${c.explanation}`).join("\n");
  const calendarLinkedIn = strategy.thirtyDayCalendar.filter((d) => d.platform === "linkedin").slice(0, 15);
...
- characterCount: leave as 0 (calculated after)`;
  ...
  for (const post of posts) {
    post.characterCount = [post.hook, post.body, post.cta, post.hashtags.join(" ")].join("\n\n").length;
  }
  return posts;
}
__name(generateLinkedInPosts, "generateLinkedInPosts");
async function generateXPosts(payload) {
  const { knowledgeBase: kb, strategy, brand } = payload;
  const calendarX = strategy.thirtyDayCalendar.filter((d) => d.platform === "x").slice(0, 30);
  const hooksFlat = [
    ...strategy.hookLibrary.curiosity,
    ...strategy.hookLibrary.controversy,
    ...strategy.hookLibrary.predictions
  ].map((h) => h.text).slice(0, 15).join("\n");
...
__name(generateXPosts, "generateXPosts");
async function generateNewsletter(payload) {
  const { knowledgeBase: kb, brand } = payload;
  const frameworksText = kb.frameworks.length > 0 ? kb.frameworks.map((f) => `${f.name}: ${f.steps.join(" → ")}. ${f.oneLiner}`).join("\n") : "None mentioned";
  const storiesText = kb.stories.length > 0 ? kb.stories.map((s) => `${s.protagonist}: ${s.lesson}`).join("\n") : "None mentioned";
...
QUOTES:
${kb.quotes.slice(0, 4).map((q) => `"${q.text}" — ${q.attributedTo}`).join("\n")}
...
- wordCount: leave as 0 (calculated after)`;
  ...
  nl.wordCount = nl.sections.flatMap((s) => [s.heading, s.body]).join(" ").split(/\s+/).filter(Boolean).length;
  return nl;
}
__name(generateNewsletter, "generateNewsletter");
var copywriterAgent = task({
  id: "copywriter-agent",
  retry: { maxAttempts: 2, factor: 2, minTimeoutInMs: 1e4, maxTimeoutInMs: 6e4 },
...
//# sourceMappingURL=chunk-HPCGOW43.mjs.map
